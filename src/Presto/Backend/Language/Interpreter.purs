...
interpret _ (ThrowException err next) = (R.lift $ S.lift $ E.ExceptT $ Left <$> (pure $ error err)) >>= pure <<< next
...
interpret (BackendRuntime a connections c) (GetCacheConn cacheName next) = do
  ...
  case maybeCache of
    ...
    Just _ -> interpret (BackendRuntime a connections c) (ThrowException "No Cache found" next)
    Nothing -> interpret (BackendRuntime a connections c) (ThrowException "No Cache found" next)
